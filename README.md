...
| 方法 | 操作 | 使用情境 |
|------|------|---------|
| **A. 拖放上傳** | 把圖檔從檔案總管直接拖到中央 **Upload Drop Zone**(綠色虛線框) | 最快,推薦 |
| **B. 點擊瀏覽** | 點 Drop Zone 任意處 → 開系統檔案選擇器 → 選圖 → 開啟 | 不方便拖放時（如圖在多層目錄） |
| **C. 用內建範例圖** | 把 `floor_plans/` 裡的圖拖進去測試 | 第一次試用、demo、驗證安裝 |

#### 支援格式
`.png` / `.jpg` / `.jpeg` / `.bmp` / `.gif`,建議解析度 **≥ 1000px**,線條清晰。

#### 檔名格式(決定建築實際尺寸)

| 檔名範例 | 系統行為 |
|---------|---------|
| `office_50x30m.png` | ✅ 自動讀取 寬 50m × 高 30m |
| `lobby_20m.png` | ⚠️ 只讀寬 20m,高度依圖片比例推算 |
| `myplan.png` | ⚠️ Fallback 寬 = 20m,需手動到側欄調 Width/Height |

> **正則**:`(\d+)\s*[xX×]\s*(\d+)\s*m` — 大小寫、`×`、空白都認。

#### 上傳後系統會做什麼
1. 後端用 OpenCV 找外牆 bounding box → **自動裁掉留白**,讓圖邊就是建築邊
2. 用裁切後的實際長寬比 + 檔名的寬度提示 → 算出 px / m 比例
3. 左側畫布顯示處理後的平面圖
4. 若重新上傳新圖,**會自動清除舊牆+舊 AP**(舊座標已不適用)
